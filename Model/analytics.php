...
function get_last_week_passenger_count(){
	global $db;
    $query = '
        SELECT COUNT(DISTINCT b.scheduleId) as totalTrips
		FROM BusTrips AS b
		INNER JOIN BusSchedules AS s ON b.scheduleId = s.scheduleId
		WHERE s.busDate BETWEEN DATE_SUB(CURDATE(), INTERVAL 7 DAY) AND CURDATE();';
    $statement = $db->prepare($query);
	
	$statement->execute();
    $last_week_bookings = $statement->fetch();
    $statement->closeCursor();
    return $last_week_bookings;
}

// last 30 days
function get_last_month_passenger_count(){
	global $db;
    $query = '
        SELECT COUNT(DISTINCT b.scheduleId) as totalTrips
		FROM BusTrips AS b
		INNER JOIN BusSchedules AS s ON b.scheduleId = s.scheduleId
		WHERE s.busDate BETWEEN DATE_SUB(CURDATE(), INTERVAL 30 DAY) AND CURDATE();
...
function get_next_week_passenger_count(){
	global $db;
    $query = '
         SELECT COUNT(DISTINCT b.scheduleId) as totalTrips
			FROM BusTrips AS b
...
function get_next_month_passenger_count(){
	global $db;
    $query = '
         SELECT COUNT(DISTINCT b.scheduleId) as totalTrips
			FROM BusTrips AS b
